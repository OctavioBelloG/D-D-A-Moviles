...
class BancoScreen extends StatelessWidget {
      ...
      appBar: AppBar(
        ...
        body:  Column(
          children: [
            context.select(
              (CuentaBloc value) =>
            Text(value.state.total.toString(), style: TextStyle(fontSize: 40)),
            ),
              Expanded(
                child: ListView.builder(
                itemCount: context.read<CuentaBloc>().state.movimientos.length,
                itemBuilder: (context, index) {
                  Movimiento mov = context
                  .read<CuentaBloc>()
                  .state
                  .movimientos[index];
                return ListTile(title: Text(mov.tipo));
                },
              ),
            ),
        ],
        ...
        floatingActionButton: Column(
          ...
          children: [
            FloatingActionButton(onPressed: () {
              ...
              // .add(Abonar(Movimiento(cantidad: 5, tipo: "abono")),
              // );
              //_showSimpleDialog(context);//metodo de abajo
            },
            child: Icon(Icons.plus_one),),
            SizedBox(height: 10,),
            FloatingActionButton(onPressed: () {
              // context.read<CuentaBloc>().add(
              //   Retirar(Movimiento(cantidad: 5, tipo: "retiro")),
              // );
              Navigator.pushNamed(context, '/retiro'); //AGREGAR VINCULO
  ...
  void _showSimpleDialog(BuildContext context) {
    showDialog(
      //crear atributos de showdialog
      context: context,//recibe el contexto
      builder: (context) {
        //regregar informacion
        //LINEA 70
        TextEditingController txtController = TextEditingController();//este controlardor se usara para obtener valores de los campos de texto
        final formKey = GlobalKey<FormState>();///VARIABLE global de la llave
        return SimpleDialog(
          title: Text("ingrese una cantidad valida"),
                    ...
                    if(formKey.currentState!.validate()){
                      ...
                      .add(Abonar(
                        Movimiento(
                          ...
                          tipo: "abono")),//se realiza un casteo por la diferencia de tipos de dato
                      //de cantidad y abono
                    );
